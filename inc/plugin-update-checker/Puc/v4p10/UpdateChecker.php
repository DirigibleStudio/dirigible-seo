...
if ( !class_exists('Puc_v4p10_UpdateChecker', false) ):
	...
	abstract class Puc_v4p10_UpdateChecker {
		...
		protected function requestMetadata($metaClass, $filterRoot, $queryArgs = array()) {
			...
			if ( !is_wp_error($status) ){
				...
				// Allow 3rd parties to suppress the notice, as they might have their own
 				...
 				if(apply_filters('puc_show_trigger_api_response_error', true)) {
					$this->triggerError(
		...
		protected function validateApiResponse($result) {
			...
			// Dirigible handle error.
			if(isset($result['body'])){
				$body = json_decode($result['body'], true);
				if(isSet($body['error'])){
					// Pass our error to error handling in dirigible-licensing
					do_action('ds_handle_api_error', $body['error']);
					return new WP_Error('ds_'.$body['error']['status'], $body['error']['body']);
				}
			}
